...
lf = cmn.log_filer(LOGDIR,'policies')
...
def main():
    policies = []
    try:
        lastEntry = mjdb.sg_last_entry('policies')
    except Exception as e:
        lf.error(f"mjdb.sg_last_entry('policies')\n{e}")
    else:
        ...
        try:
            batchesStatement = f"SELECT POLICIES *CRITERIA.BATCH* WITH LAST.ENTRY.DATE GE {dt.datetime.strftime(lastEntryDate, '%m-%d-%Y')}"
            ...
            for batch in hlp.parse_batch_items(batchesResponse):
                try:
                    batchStatement = f"SELECT POLICIES *GET.BATCH* {batch}"
        ...
        # row list to dataframe, stage dataframe in database
        try:
